mutable struct RelaxedSDPWRMPowerModel <: PM.AbstractWRMModel PM.@pm_fields end
function build_opf_mod(pm::RelaxedSDPWRMPowerModel)
...
function collect_split_vars(pm::RelaxedSDPWRMPowerModel)
    wr = PM.var(pm, :wr)
    wi = PM.var(pm, :wi)
    w  = PM.var(pm,  :w)
    p  = PM.var(pm,  :p)
